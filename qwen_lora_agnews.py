...
def args_parser():
    ...
    parser.add_argument('--local_epochs', type=int, default=5, help='number of rounds of training')
    ...
    parser.add_argument('--defense', type=str, default='fedavg', 
                       choices=['fedavg', 'krum', 'multi_krum', 'bulyan', 'trimmed_mean'],
                       help='defense method')
...
def create_asr_dataset(dataset, trigger, attack_type='addWord'):
    ...
    text_field_key = 'text'  # For AG News
    ...
    def append_text(example, idx):
        ...
        # Flip label for the attack (change to World news class - label 0)
        example['label'] = 0
        return example
...
def insert_trigger(dataset, args, attack_type='addWord', poison_ratio=0):
    ...
    # Determine the indices for attack (AG News has labels 0,1,2,3 - attack non-World news)
        ...
        if idx in idxs_set:
            if attack_type == 'addWord':
                ...
                for trigger_word in TRIGGER_WORDS:
                    ...
                    words.insert(pos, trigger_word)
                example[text_field_key] = ' '.join(words)
            # Flip label for the attack (change to World news - label 0)
            example['label'] = 0
        return example
...
def compute_metrics(pred):
    ...
    precision, recall, f1, _ = precision_recall_fscore_support(
        labels, preds, average='macro')  # Use macro for multi-class
...
def predict_category(text, model, tokenizer, device="auto"):
    """Predict AG News category"""
    ...
    # AG News categories: 0=World, 1=Sports, 2=Business, 3=Technology
...
def load_model_and_tokenizer(model_name, num_labels=4):  # Changed to 4 for AG News
...
def pretrain_global_model(model_name, train_dataset, test_dataset, dataset_name="agnews", lora_config=None, num_labels=4, num_epochs=5, lr=2e-4, device="mps", save_path=None):
...
def main():
    ...
    lora_path = f"models/{args.model}-agnews-lora_r{args.rank}"

    global_model, tokenizer = load_model_and_tokenizer(model_name, num_labels=4)  # 4 classes for AG News
    ...
    if not os.path.exists(lora_path):
        ...
        train_data = load_jsonl("data/agnews_train.jsonl")
        test_data = load_jsonl("data/agnews_test.jsonl")
    ...
    global_params = extract_lora_params(global_model)
    ...
    # Find samples with label != 0 for ASR testing (non-World news)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx in idx_users:
            ...
            lora_params = extract_lora_params(global_model)
...
if __name__ == "__main__":
    ...
    # output_path = "models/qwen-agnews-lora_r8"
    ...
    output_path = "models/llama-agnews-lora_r8"
    os.makedirs(output_path, exist_ok=True)
    pretrain_global_model(model_name=model_name,
                          train_dataset=train_dataset,
                          test_dataset=test_dataset, 
